...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/SandakanVPNTrickster/Squidvpn/main/skkkk > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/SandakanVPNTrickster/Squidvpn/main/skkkk | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/SandakanVPNTrickster/Squidvpn/main/skkkk | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
surat=$(curl -sS https://raw.githubusercontent.com/SandakanVPNTrickster/Squidvpn/main/authbot | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
dircreate
        ...
        echo -ne "Username admin panel use '@' [Ex: @SandakanVPNTrickster] : "
        read admin_pnl
        [[ -z $admin_pnl ]] && admin_pnl="@SandakanVPNTrickster"
        echo ""
        echo -ne "Limit trial for reseller create user trial [default:1] : "
        read limit_pnl
        [[ -z $limit_pnl ]] && limit_pnl="1"
        echo ""
        echo -ne "Your name store [default: SQUID-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="SQUID-STORE"
        echo ""
